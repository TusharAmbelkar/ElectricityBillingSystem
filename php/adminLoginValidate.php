<?php  
	...
	$conn=dbCon();
	$stmt = $conn->prepare("SELECT firstName,middleName,lastName,email,userName,password,mobilenumber1,cashCredit from admin WHERE username = ? && password = ?");
	$stmt->bind_param("ss",$username,$password);
	$username=$_POST['username'];
	$password=$_POST['password'];
	$stmt->execute();
	$stmt->bind_result($firstname,$middlename,$lastname,$email,$userName,$password,$contactnumber,$cashCredit);
	$stmt->store_result();
	if ($stmt->num_rows==0) {
